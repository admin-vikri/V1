...
import { testDhaba, testDhaba_categories } from "../../../items/TestDhaba";
import MealItem from "../../Meals/MealItem/MealItem";

const TestDhaba = () => {
  const [list, setList] = useState();
  const cartCtx = useContext(CartContext);
  const [filteredList, setFilteredList] = new useState(testDhaba_categories);
  let mealss;
  ...
  useEffect(() => {
    if (cartCtx.isVeg === "veg") {
      mealss = testDhaba.filter((item) => item.veg === cartCtx.isVeg);
  ...
  const getApiData = async () => {
    ...
    console.log(testDhaba);
    setList(response);
  };

  const categories = testDhaba_categories;
  if (cartCtx.isVeg === "veg") {
    mealss = testDhaba.filter((item) => item.veg === cartCtx.isVeg);
  } else if (cartCtx.isVeg === "nonveg") {
    mealss = testDhaba.filter((item) => item.veg === cartCtx.isVeg);
